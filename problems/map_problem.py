...
class MapProblem(GraphProblem):
    ...
    def expand_state_with_costs(self, state_to_expand: GraphProblemState) -> Iterator[OperatorResult]:
        ...
        if junction.all_connected_links is None:
            return None

        for link in junction.all_connected_links:
            # target_junction = self.streets_map[link.target]
            target_map_state = MapState(link.target)
            yield OperatorResult(successor_state=target_map_state, operator_cost=link.distance)
